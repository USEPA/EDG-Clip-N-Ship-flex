...
package widgets.FrClipAndShip.toc.tocClasses
{
	...
	public class TocItem extends EventDispatcher
	{
	    ...
	    {
	        _parent = parentItem;
			// Added by Fr per post http://forums.arcgis.com/threads/20965-LiveMapsWidget-doesn-t-follow-changes
	    ...
	    public function set label(value:String):void
	    {
			...
			// Used to display a list of the items in the TOC on Load
			//trace ("Value from TocItem.as set label: " + value);
	     ...
	     */	
		// by fr - following 2 lines
		//private var layerClicked:String;		
		//[Bindable(event="layerClicked")]		
	    internal function setVisible(value:Boolean, layerRefresh:Boolean = true):void		
	    {
			// Used to display visiblity and layer name
			//trace ("TocItem.as setVisible: " + value);
			//trace ("working on: " + _label);			
			
			// Added by Frank to communicate with FrClipAndShipWidget.mxml to pass TOC change in layers			
			var myevent:MyOwnEvent = new MyOwnEvent(MyOwnEvent.RECEIVED_EVENT);			
			myevent.layerName = _label;
			myevent.layerOn = value.toString();
